...
public:
    ...
    allocation *bestFit(std::size_t size);
};
...
void *allocBestFit(std::size_t chunk_size); 
...
allocation *LinkedList::bestFit(std::size_t size)
{
    Node *current = head;
    Node *bestNode = nullptr;
    std::size_t minDiff = 999999999;

    while (current != nullptr)
    {
        std::size_t diff = current->alloc->size - size;
    ...
    return bestNode ? bestNode->alloc : nullptr;
}
...
void *allocBestFit(std::size_t chunk_size)
{
    ...
    allocation *alloc = freeList.bestFit(alloc_size);
    if (alloc != nullptr)
    {
